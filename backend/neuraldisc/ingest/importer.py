"""High-throughput media importer.

Pipeline (max media/hour, limited by source read speed):
  1. Scan source(s) and pre-filter junk (quality gates)
  2. Parallel COPY into library/staging/{provenance}/  (I/O bound)
  3. Parallel PROCESS & classify while still in staging
  4. PROMOTE to originals/ only after success; purge rejects from staging

API returns immediately; work runs in a background thread with Job progress.
"""
...
class ImportProgress:
    ...
    def items_per_hour(self) -> float:
        ...
        done = self.promoted + self.rejected
...
def start_import(
    ...
    *,
    ...
    """Create job row and start background import. Returns job_id."""
...
def _run_import(job_id: str, sources: list[ImportSource], settings: Settings) -> None:
    progress = get_import_progress(job_id)
    assert progress is not None
    progress.status = "running"
    progress.phase = "scanning"
    progress.started_at = time.time()
    progress.message = "Scanning sources…"
    ...
    try:
        ...
        for src in sources:
            ...
            _import_one_source(src, settings, progress)
        ...
        progress.message = (
            ...
            f"{progress.errors} errors · ~{progress.items_per_hour:.0f} items/h"
        )
...
def _import_one_source(
    ...
    # --- Parallel copy → process pipeline ---
    progress.phase = "copying"
    copy_workers = max(1, settings.import_copy_workers)
    process_workers = max(1, settings.import_process_workers)
    process_q: Queue[str | None] = Queue(maxsize=process_workers * 4)
    stop_token = None
    ...
    if _cancelled(progress):
        # Stop processors and surface cancellation
        for _ in processors:
